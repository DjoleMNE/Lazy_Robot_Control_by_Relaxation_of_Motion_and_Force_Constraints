...
void ChainHdSolver_Vereshchagin::initial_upwards_sweep(const JntArray &q, const JntArray &qdot, const JntArray &qdotdot, const Wrenches& f_ext)
{
    ...
    std::cout << "FPK(ee):" << std::endl;
    std::cout << F_total << std::endl;
...
#define TRANSFORM_CONSTRAINT
#ifdef TRANSFORM_CONSTRAINT
                col = base_to_end*col;
#endif
        ...
        if (i != 0)
        {
            ...
            std::cout << "ZtE[" << j << "]=" << s.EZ.transpose().format(fmt) << std::endl;
...
void ChainHdSolver_Vereshchagin::constraint_calculation(const JntArray& beta)
{
    ...
    /*
    std::cout << "constraint force (A):" << std::endl;
    std::cout << results[0].E << std::endl;

    std::cout << "generated acc. energy (U):" << std::endl;
    std::cout << results[0].G << std::endl;

    std::cout << "required acc. energy (L):" << std::endl;
    ...
    */
    ...
    const IOFormat fmt(1, DontAlignCols, "\t", " ", "", "", "[", "]");
    std::cout << std::endl;
    std::cout << "Sm: " << Sm.transpose().format(fmt) << std::endl;
    std::cout << "nu: " << nu.transpose().format(fmt) << std::endl;
}
...
void ChainHdSolver_Vereshchagin::final_upwards_sweep(JntArray &q_dotdot, JntArray &torques)
{
    ...
    std::cout << std::endl;
    std::cout << "FAK(ee):" << std::endl;
    std::cout << results[ns].F_base.M * results[ns].acc << std::endl;
}
