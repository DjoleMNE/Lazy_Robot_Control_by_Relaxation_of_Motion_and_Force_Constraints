...
for sample_ in range(1, rows - 1, 3):
...
for sample_ in range(2, rows - 1, 3):
...
# estimated external torques
for sample_ in range(3, rows - 1, 3):
    joint_0_ext_2.append( np.float32( input_data[sample_][0] ) )
    joint_1_ext_2.append( np.float32( input_data[sample_][1] ) )
    joint_2_ext_2.append( np.float32( input_data[sample_][2] ) )
    joint_3_ext_2.append( np.float32( input_data[sample_][3] ) )
    joint_4_ext_2.append( np.float32( input_data[sample_][4] ) )
    if (joint_num > 5):
        joint_5_ext_2.append( np.float32( input_data[sample_][5] ) )
        joint_6_ext_2.append( np.float32( input_data[sample_][6] ) )

samples = np.arange(0, rows / 3, 1)
joint_0 = np.array(joint_0) * 0.0
joint_1 = np.array(joint_1) * 0.0
joint_2 = np.array(joint_2) * 0.0
joint_3 = np.array(joint_3) * 0.0
joint_4 = np.array(joint_4) * 0.0
joint_5 = np.array(joint_5) * 0.0
joint_6 = np.array(joint_6) * 0.0
...
joint_0_ext_2 = np.array(joint_0_ext_2)
joint_1_ext_2 = np.array(joint_1_ext_2)
joint_2_ext_2 = np.array(joint_2_ext_2)
joint_3_ext_2 = np.array(joint_3_ext_2)
joint_4_ext_2 = np.array(joint_4_ext_2)
joint_5_ext_2 = np.array(joint_5_ext_2)
joint_6_ext_2 = np.array(joint_6_ext_2)
...
plt.margins(0,0)
plt.suptitle('Total Joint Torques', fontsize=20)

plt.subplot(joint_num, 1, 1)
t2 = plt.plot(joint_0_ext, c = 'purple', linewidth = 1.5, zorder = 2)
t3 = plt.plot(joint_0_ext_2, c = 'orange', linewidth = 1.5, zorder = 2)
...
plt.plot(joint_1_ext_2, c = 'orange', linewidth = 1.5, zorder = 2)
...
plt.plot(joint_2_ext_2, c = 'orange', linewidth = 1.5, zorder = 2)
...
plt.plot(joint_3_ext_2, c = 'orange',  linewidth = 1.5, zorder = 2)
...
plt.plot(joint_4_ext_2, c = 'orange', linewidth = 1.5, zorder = 2)
# plt.axvline(x = 1975.0, linewidth = 1.3,  color='blue', zorder = 1)
...
if (joint_num > 5):
    ...
    plt.plot(joint_5_ext_2, c = 'orange', linewidth = 1.5, zorder = 2)
    # plt.axvline(x = 1975.0, linewidth = 1.3,  color='blue', zorder = 1)
    ...
    plt.plot(joint_6_ext_2, c = 'orange', linewidth = 1.5, zorder = 2)
...
plt.figlegend((t2[0], t3[0]), ("Total torque Generic", "Total torque Vereshchagin"), loc=1, fontsize = 12)
plt.draw()
plt.pause(0.001)
plt.savefig('../experiments/joint_total_torques.pdf')
